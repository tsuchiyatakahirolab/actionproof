...
- Added a dated competitive review and v0.6 release specification with a bounded one-sentence difference, then superseded it with v0.7 product evidence.
...
- Removed a premature visual `PASS` during repaired-run animation; the lifecycle now shows `VERIFYING` until the identical regression actually completes. The suite now includes 28 unit and 9 native-browser tests, including exactly-once write coverage for both native input dialects, external consumer execution/replay, identity-set delimiter-collision protection, and automated WCAG A/AA checks in initial and blocked states.
