...
- **Package evidence:** typed ESM public API, zero runtime dependencies, 10,804-byte bundle, generated declarations, strict NodeNext consumer typecheck, an external built-package consumer returning `ACTION_PROVEN`, and a dry-run 15.7 KB package manifest. The demo invokes the same public gate.
- **Delivery quality:** a deterministic build gate measures 70,524 gzip bytes of emitted JavaScript and 5,942 gzip bytes of CSS, rejects external runtime assets and source maps, and validates social metadata. Three declared-condition cold Chrome desktop runs passed with worst TTFB 31.2 ms, FCP/LCP 1,444 ms, TBT 0 ms, CLS 0.0007, zero cross-origin runtime requests, zero automated WCAG A/AA violations, and zero console errors. One-shot visual sequencing never lowers text opacity and respects reduced motion. These are bounded lab results, not field data or a Lighthouse score.
...
- **Integration evidence:** an external consumer uses the built SDK without the demo store; an application supplies visible intent, an independent snapshot, and its WebMCP action binding.
