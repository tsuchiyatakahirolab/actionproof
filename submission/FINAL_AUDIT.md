...
| `npm run audit:runtime` | PASS — release-window three-run cold Chrome desktop gate at declared 40 ms / 10 Mbps / 1x CPU; worst TTFB 41.4 ms, FCP/LCP 1,564 ms, TBT 0 ms, CLS 0.0007; zero cross-origin runtime requests, automated WCAG A/AA violations, or console errors |
...
| `npm run audit:production` | PASS — public production exposes native WebMCP, both context-matched tools, both external-call gates, executable regressions, both defect detections, both identical repair passes, valid social card, `tools=*`, and zero console errors |
...
| `git diff --check` | PASS |
| GitHub Actions CI | PASS — release revision workflow completed successfully |

## Native Chrome evidence for the release build
...
`submission/PRODUCTION_AUDIT.json` records the release-day public audit: native WebMCP, both context-matched tools, direct external calls entering the gate, executable regression artifacts, both defect detections, both identical regression passes, `tools=*`, a valid PNG social card, and zero console errors.
...
- Three independent cold Chrome desktop contexts were measured at 1440×900 with 40 ms latency, 10 Mbps down, 5 Mbps up, and 1x CPU. The final release-window gate uses the worst run: TTFB 41.4 ms, FCP/LCP 1,564 ms, TBT 0 ms, and CLS 0.0007.
...
- 16:9 upload thumbnail: `submission/youtube-thumbnail-v2.png` (deterministically rendered from the native release build after the external seeded call; the hero Effect Trace and release decision are visible in one frame)
